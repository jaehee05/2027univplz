...
import type { Assignment, Answer } from "@/lib/types/work";
import { answerRef, assignmentRef, toAnswer, toAssignment } from "@/lib/work/store";

type Loaded =
  | { ok: true; input: CorrectionInput; assignment: Assignment; answer: Answer }
...
export async function loadCorrectionInput(
  ...
  if (!assignment.answerId) {
    return { ok: false, status: 400, error: "아직 답안이 없습니다." };
  }
  ...
  if (!answer || answer.status !== "submitted") {
    return { ok: false, status: 400, error: "학생이 아직 제출하지 않았습니다." };
  }
  ...
  if (!univSnap.exists || !questionSnap.exists) {
    return { ok: false, status: 404, error: "문항을 찾지 못했습니다." };
  }

...
/**
 * 이미 Firebase 계정이 있으면 로그인으로 이어붙인다.
 * 이전 시도에서 계정만 만들어지고 서버 등록이 실패한 경우를 되살리기 위한 것.
 */
export async function emailSignUpOrSignIn(email: string, password: string): Promise<User> {
  try {
    const credential = await emailSignUp(email, password);
...
export async function exchangeSession(user: User, forceRefresh = false): Promise<Response> {
